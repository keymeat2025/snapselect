...
const GalleryShareModal = {
  ...
  updateRevokeButtonClass: function() {
    ...
    if (this.revokeAttempts === 2) {
      ...
      revokeBtn.textContent = 'All revoke attempts used';
  ...
  displayShareLink: function(shareId) {
    ...
    // Show revoke button and update its state based on current attempts from database
    const revokeBtn = document.getElementById('revokeAccessBtn');
    if (revokeBtn) {
  ...
  // Revoke access to a shared gallery with simple 2-attempt system
  revokeAccess: function() {
    try {
      ...
      if (this.revokeAttempts <= 0) {
        this.showToast('You have used all your revoke attempts for this gallery.', 'error');
        return;
      }
      ...
      db.collection('galleryShares')
        ...
        .then(snapshot => {
          ...
          return db.collection('galleryShares').doc(shareDoc.id).update({
            ...
            // Show success message
            self.showToast('Gallery access revoked successfully. You have used all your revoke attempts.', 'success');
