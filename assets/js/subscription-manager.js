...
window.subscriptionManager = {
  ...
  checkGalleryDeletionStatus,
  getContractualPeriod,
  updateDeleteButton,
  handleDeleteClick,
  executeGalleryDeletion,
...
function updatePlansDisplay(plans) {
  ...
  document.querySelectorAll('.edit-share-btn').forEach(btn => {
   ...
   // Initialize delete buttons (ADD THIS)
  setTimeout(() => {
    initializeDeleteButtons();
  }, 100);
}
...
// Smart Gallery Delete Manager - Add these functions to your subscription-manager.js

/**
 * Gallery Delete Manager
 * Add these functions to your existing subscription-manager.js file
 */

/**
 * Check if a gallery can be deleted based on business rules
 * @param {Object} plan - Plan data object from your allPlans array
 * @returns {Object} - {canDelete: boolean, reason: string, type: string}
 */
function checkGalleryDeletionStatus(plan) {
    const now = new Date();
    ...
    // Calculate contractual period based on plan type  
    const contractualDays = getContractualPeriod(planType);
    const contractualEndDate = plan.deliveredDate ? 
        new Date(new Date(plan.deliveredDate).getTime() + contractualDays * 24 * 60 * 60 * 1000) : null;
    
    // Get client info
    const client = userClients.find(c => c.id === plan.clientId);
    
    // 🔴 NON-DELETABLE CONDITIONS (in order of priority)
    
    // Recent client activity (within 24 hours)
    if (plan.lastClientAccess && new Date(plan.lastClientAccess) > oneDayAgo) {
        return {
            canDelete: false,
            reason: "Cannot delete: Recent client activity - Accessed in past 24 hours",
            type: "recent_activity"
        };
    }
    
    // Client has active access (currently shared and not expired)
    if (plan.shareUrl && !plan.shareExpired && planStatus === PLAN_STATUS.ACTIVE) {
        return {
            canDelete: false,
            reason: "Cannot delete: Client has active access - Currently shared",
            type: "active_access"
        };
    }
    
    // Pending client selections (shared but selections not completed)
    if (plan.shareUrl && !plan.selectionsCompleted && !plan.shareExpired) {
        return {
            canDelete: false,
            reason: "Cannot delete: Pending client selections - Client hasn't selected",
            type: "pending_selections"
        };
    }
    
    // Within active plan period
    if (planStatus === PLAN_STATUS.ACTIVE) {
        return {
            canDelete: false,
            reason: "Cannot delete: Within plan period - Active subscription",
            type: "active_plan"
        };
    }
    
    // Within contractual obligation period
    if (contractualEndDate && now < contractualEndDate) {
        const daysLeft = Math.ceil((contractualEndDate - now) / (24 * 60 * 60 * 1000));
        return {
            canDelete: false,
            reason: `Cannot delete: Contractual obligation period - ${daysLeft} days remaining`,
            type: "contractual_period"
        };
    }
    
    // 🟢 DELETABLE CONDITIONS
    
    // Empty gallery (no photos uploaded)
    if (!plan.photosUploaded || plan.photosUploaded === 0) {
        return {
            canDelete: true,
            reason: "Delete empty gallery - No photos uploaded",
            type: "empty_gallery"
        };
    }
    ...
    if (!plan.shareUrl) {
        return {
            canDelete: true,
            reason: "Delete unshared gallery - Never shared with client",
            type: "unshared_gallery"
        };
    }
    
    // Completed project (selections done and delivered, past contractual period)
    if (plan.selectionsCompleted && plan.delivered && 
        contractualEndDate && now > contractualEndDate) {
        return {
            canDelete: true,
            reason: "Delete completed project - Client selections done, delivered",
            type: "completed_project"
        };
    }
    
    // Past retention period (for expired plans)
    if (planStatus === PLAN_STATUS.EXPIRED && plan.pastRetentionPeriod) {
        return {
            canDelete: true,
            reason: "Delete expired gallery - Past retention period",
            type: "expired_gallery"
        };
    }
    ...
    return {
        canDelete: false,
        reason: "Cannot delete: Contact support for assistance",
        type: "unknown"
    };
}

/**
 * Get contractual obligation period based on plan type
 * Uses the retention periods you defined: 7, 14, 21 days
 */
function getContractualPeriod(planType) {
    const periods = {
        'lite': 7,
        'mini': 7,
        'basic': 14,
    ...
    return periods[planType] || 14; // Default to 14 days
}

/**
 * Update a delete button's appearance and functionality
 * @param {HTMLElement} buttonElement - The delete button element
 * @param {Object} plan - Plan data object
 */
function updateDeleteButton(buttonElement, plan) {
    if (!buttonElement) return;

    const status = checkGalleryDeletionStatus(plan);
    let tooltip = buttonElement.querySelector('.delete-btn-tooltip');
    
    // Create tooltip if it doesn't exist
    if (!tooltip) {
        tooltip = document.createElement('div');
        tooltip.className = 'delete-btn-tooltip';
        buttonElement.appendChild(tooltip);
    }
    
    // Remove existing state classes
    buttonElement.classList.remove('deletable', 'non-deletable', 'loading', 'auto-delete-warning');
    
    // Update tooltip text
    tooltip.textContent = status.reason;
    
    // Apply appropriate state
    if (status.canDelete) {
        buttonElement.classList.add('deletable');
        buttonElement.disabled = false;
        buttonElement.style.cursor = 'pointer';
    } else {
        buttonElement.classList.add('non-deletable');
        buttonElement.disabled = true;
        buttonElement.style.cursor = 'not-allowed';
    }
    
    // Store status data for click handler
    buttonElement.dataset.canDelete = status.canDelete;
    buttonElement.dataset.deleteReason = status.reason;
    buttonElement.dataset.deleteType = status.type;
    buttonElement.dataset.planId = plan.id;
    buttonElement.dataset.clientId = plan.clientId;
}
 ...
 * @param {Object} plan - Plan data object
 */
function handleDeleteClick(buttonElement, plan) {
    const canDelete = buttonElement.dataset.canDelete === 'true';
    
    if (!canDelete) {
        // Show why deletion is not allowed using notification system
        if (window.NotificationSystem) {
            window.NotificationSystem.showNotification(
                'warning',
                'Cannot Delete Gallery',
                buttonElement.dataset.deleteReason
            );
        } else {
            showErrorMessage(buttonElement.dataset.deleteReason);
        }
        return;
    }
    
    // Show confirmation dialog
    const deleteType = buttonElement.dataset.deleteType;
    const reason = buttonElement.dataset.deleteReason;
    const client = userClients.find(c => c.id === plan.clientId);
    const clientName = client?.name || plan.clientName || 'Unknown Client';
    
    showDeleteConfirmationModal(plan, clientName, reason, deleteType);
}

/**
 * Show custom delete confirmation modal
 */
function showDeleteConfirmationModal(plan, clientName, reason, deleteType) {
    ...
    const warningBox = modal.querySelector('.delete-warning-box');
    
    title.textContent = 'Delete Gallery Confirmation';
    
    message.innerHTML = `
        <strong>Gallery:</strong> ${plan.galleryId || 'Gallery'} for client <strong>${clientName}</strong><br>
        <strong>Reason:</strong> ${reason}
    `;
    
    // Set warning based on delete type
    if (deleteType === 'completed_project') {
        warningBox.innerHTML = `
            <strong>⚠️ Warning:</strong> This gallery contains completed client selections. 
            Make sure you have downloaded and delivered all selected photos before deleting.
        `;
    } else if (deleteType === 'empty_gallery') {
        warningBox.innerHTML = `
            <strong>ℹ️ Info:</strong> This gallery is empty and can be safely deleted.
        `;
    } else {
        warningBox.innerHTML = `
            <strong>⚠️ Warning:</strong> This action cannot be undone. All photos and client data will be permanently deleted.
        `;
    }
    ...
    confirmBtn.onclick = () => {
        ...
        executeGalleryDeletion(plan);
    };
    
    // Show modal
    modal.style.display = 'block';
    setTimeout(() => {
        modal.querySelector('.delete-confirmation-content').classList.add('show');
    }, 10);
}
    ...
    modal.innerHTML = `
        <div class="delete-confirmation-content">
            <div class="delete-confirmation-header">
                ...
                <h3 class="delete-confirmation-title">Delete Gallery</h3>
            </div>
            <div class="delete-confirmation-body">
                <div class="delete-confirmation-message"></div>
                <div class="delete-warning-box"></div>
                <div class="delete-confirmation-actions">
                    <button class="delete-cancel-btn">Cancel</button>
                    <button class="delete-confirm-btn">Delete Gallery</button>
                </div>
            </div>
        </div>
    `;
...
/**
 * Execute the actual gallery deletion
 */
async function executeGalleryDeletion(plan) {
    try {
        showLoadingOverlay('Deleting gallery...');
        ...
        // Step 1: Delete gallery document if it exists
        if (plan.galleryId) {
            await db.collection('galleries').doc(plan.galleryId).delete();
            console.log('Gallery document deleted:', plan.galleryId);
        }
        ...
        console.log(`Deleted ${photoDeletePromises.length} photos`);
        
        // Step 3: Delete photo selections
        const selectionsSnapshot = await db.collection('photoSelections')
            .where('galleryId', '==', plan.galleryId)
            .get();
        
        const selectionDeletePromises = selectionsSnapshot.docs.map(doc => doc.ref.delete());
        await Promise.all(selectionDeletePromises);
        console.log(`Deleted ${selectionDeletePromises.length} photo selections`);
        
        // Step 4: Delete gallery shares
        if (plan.shareId) {
            const shareSnapshot = await db.collection('galleryShares')
                .where('shareId', '==', plan.shareId)
                .get();
            
            const shareDeletePromises = shareSnapshot.docs.map(doc => doc.ref.delete());
            await Promise.all(shareDeletePromises);
            console.log('Deleted gallery shares');
        }
        
        // Step 5: Update the plan to remove gallery reference
        await db.collection('client-plans').doc(plan.id).update({
            galleryId: firebase.firestore.FieldValue.delete(),
        ...
        if (window.NotificationSystem) {
            window.NotificationSystem.showNotification(
                'success',
                'Gallery Deleted',
                `Gallery for ${clientName} has been successfully deleted.`
            );
        } else {
            showSuccessMessage(`Gallery for ${clientName} deleted successfully`);
        }
        ...
        if (window.NotificationSystem) {
            window.NotificationSystem.showNotification(
                'error',
                'Deletion Failed',
                `Error deleting gallery: ${error.message}`
            );
        } else {
            showErrorMessage(`Error deleting gallery: ${error.message}`);
        }
    }
}

/**
 * Initialize delete buttons for all plans
 * Call this function after updatePlansDisplay() in your existing code
 */
function initializeDeleteButtons() {
    document.querySelectorAll('.delete-btn').forEach(button => {
        ...
        if (plan) {
            // Update button state based on business rules
            updateDeleteButton(button, plan);
            
            // Add click event listener
            button.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                handleDeleteClick(this, plan);
            });
        }
    });
}
...
/**
 * Update the getDownloadButtonHTML function in your existing code
 * to include the delete button
 */
function getActionButtonsHTML(plan) {
    const isActivePlan = plan.status === PLAN_STATUS.ACTIVE || plan.status === PLAN_STATUS.EXPIRING_SOON;
    ...
    // View Gallery Button
    buttonsHTML += `
        <button class="btn view-gallery-btn" data-client-id="${plan.clientId}">
    ...
    buttonsHTML += `
        <button class="btn delete-btn" 
            ...
            <div class="delete-btn-tooltip">Checking deletion status...</div>
        </button>
    `;
    
    // Extend/Renew Button (existing logic)
    if (plan.status === PLAN_STATUS.EXPIRED) {
        buttonsHTML += `
            <button class="btn renew-plan-btn" data-plan-id="${plan.id}" data-client-id="${plan.clientId}">
                <i class="fas fa-refresh"></i> Renew Plan
            </button>
        `;
    } else {
        buttonsHTML += `
            <button class="btn extend-plan-btn" data-plan-id="${plan.id}" data-client-id="${plan.clientId}">
                <i class="fas fa-plus"></i> Extend Plan
            </button>
        `;
    }
 ...
 * 
 * 1. Add the CSS from the previous artifact to your dashboard.css file
 * 
 * 2. Add these functions to your subscription-manager.js file
 * 
 * 3. In your updatePlansDisplay() function, replace the getDownloadButtonHTML(plan) 
 *    call with getActionButtonsHTML(plan) in the table cell:
 *    
 *    Change this:
 ...
 *    ${getActionButtonsHTML(plan)}
 * 
 * 4. After you create the table in updatePlansDisplay(), add this call:
 *    setTimeout(() => {
 *        initializeDeleteButtons();
 *    }, 100);
 * 
 * 5. The delete buttons will automatically show green (deletable) or red (non-deletable)
 *    based on the business rules we discussed
 */
