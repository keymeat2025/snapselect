...
function validateMimeType(file) {
  ...
  return {
    isValid: allowedMimeTypes.includes(file.type.toLowerCase()),
    mimeType: file.type,
    error: allowedMimeTypes.includes(file.type.toLowerCase()) 
      ? null 
...
// ============================================================================
// 2. FILE HEADER VALIDATION (15 minutes)
// ============================================================================

/**
 * Validates file by checking magic bytes (file signature)
 * This prevents files with fake extensions
 */
async function validateFileHeader(file) {
  try {
    ...
    // File signature patterns (magic bytes)
    const signatures = {
      'image/jpeg': [
        [0xFF, 0xD8, 0xFF], // JPEG
      ],
    ...
    return {
      ...
      error: `File header doesn't match any supported image format. This may be a corrupted file or a non-image file with an image extension.`
    };
    ...
    return {
      isValid: false,
      detectedType: 'error',
      error: `Could not read file header: ${error.message}`
    };
  }
}

/**
 * Helper function to match byte patterns
 */
...
// ============================================================================
// 3. ENHANCED ERROR MESSAGES (10 minutes)
// ============================================================================

/**
 * Generates detailed, user-friendly error messages
 */
function getDetailedValidationError(file, errorType, details = {}) {
  const errorMessages = {
    'invalid_mime_type': {
      title: 'Invalid File Type',
      message: `"${file.name}" is not a supported image format.`,
      suggestion: 'Please select only JPEG, PNG, GIF, or WebP images.',
      technicalDetails: `File type detected: ${details.detectedType || file.type}`
    },
    
    'invalid_file_header': {
      title: 'Corrupted or Invalid File',
      message: `"${file.name}" appears to be corrupted or is not a real image file.`,
      suggestion: 'Try re-saving the image in a standard format, or choose a different file.',
      technicalDetails: `File header validation failed`
    },
    
    'file_too_large': {
      title: 'File Too Large',
      message: `"${file.name}" exceeds the maximum file size limit.`,
      suggestion: `Please reduce the file size to under ${details.maxSize}MB or compress the image.`,
      technicalDetails: `File size: ${formatFileSize(file.size)}, Limit: ${details.maxSize}MB`
    },
    
    'plan_limit_exceeded': {
      title: 'Storage Limit Reached',
      message: `Cannot upload "${file.name}" - your plan's photo limit has been reached.`,
      suggestion: 'Please upgrade your plan or delete some existing photos to make space.',
      technicalDetails: `Plan limit: ${details.planLimit} photos, Current: ${details.currentCount} photos`
    },
    
    'duplicate_file': {
      title: 'Duplicate File Detected',
      message: `"${file.name}" has already been uploaded to this gallery.`,
      suggestion: 'This file appears to be identical to an existing photo. Choose a different file or check if it was already uploaded.',
      technicalDetails: `Matched existing file: ${details.existingFileName || 'Unknown'}`
    },
    
    'server_validation_failed': {
      title: 'Server Validation Failed',
      message: `"${file.name}" could not be validated by the server.`,
      suggestion: 'Please try again. If the problem persists, contact support.',
      technicalDetails: details.serverError || 'Unknown server error'
    }
  };
  ...
  const error = errorMessages[errorType] || {
    ...
    technicalDetails: 'Unknown validation error'
  };
...
function showEnhancedErrorMessage(errorInfo) {
  const message = `
    <div class="enhanced-error">
      <h4>${errorInfo.title}</h4>
      <p><strong>${errorInfo.message}</strong></p>
      <p class="suggestion">${errorInfo.suggestion}</p>
      <details class="technical-details">
        <summary>Technical Details</summary>
        <p>${errorInfo.technicalDetails}</p>
      </details>
    </div>
  `;
  
  // Use your existing error message system
...
/**
 * Validates files with server-side checking
 * This adds an extra layer of security validation
 */
async function validateWithServer(files, galleryId) {
  try {
    // Prepare file metadata for server validation
    ...
    // Call Firebase Cloud Function for validation
    const functions = firebase.app().functions('asia-south1');
    const validateFiles = functions.httpsCallable('validateFiles');
    ...
    return {
      ...
      validFiles: result.data.validFiles || [],
      invalidFiles: result.data.invalidFiles || [],
    ...
    // Don't block upload if server validation fails
    // Just log the error and continue with client-side validation
    return {
      isValid: true, // Allow upload to continue
      validFiles: files.map(f => f.name),
      invalidFiles: [],
      errors: [`Server validation unavailable: ${error.message}`],
      warnings: ['Proceeding with client-side validation only']
    };
  }
}

// ============================================================================
// 5. COMPREHENSIVE FILE VALIDATION FUNCTION
// ============================================================================

/**
 * Master validation function that combines all validation checks
 * REPLACE your existing file validation logic with this
 */
async function validateFilesComprehensively(files, galleryId, planLimits) {
  const validFiles = [];
  const rejectedFiles = [];
  const warnings = [];
  
  console.log(`🔍 Starting comprehensive validation for ${files.length} files...`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      // 1. Basic file type filter
      if (!file.type.startsWith('image/')) {
        isValid = false;
        errorType = 'invalid_mime_type';
        rejectionReason = 'Not an image file';
      }
      ...
      if (isValid) {
        ...
        if (!mimeValidation.isValid) {
          isValid = false;
          errorType = 'invalid_mime_type';
          rejectionReason = mimeValidation.error;
          errorDetails.detectedType = mimeValidation.mimeType;
        }
      }
      ...
      if (isValid) {
        ...
        if (!headerValidation.isValid) {
          isValid = false;
          errorType = 'invalid_file_header';
          rejectionReason = headerValidation.error;
          errorDetails.detectedType = headerValidation.detectedType;
        }
      }
      
      // 4. File size validation
      if (isValid && planLimits) {
        const maxSizeBytes = planLimits.maxSize * 1024 * 1024;
        if (file.size > maxSizeBytes) {
          isValid = false;
          errorType = 'file_too_large';
          rejectionReason = `Exceeds size limit of ${planLimits.maxSize}MB`;
          errorDetails.maxSize = planLimits.maxSize;
        }
      }
      
      // 5. Plan limits validation
      if (isValid && planLimits && galleryData) {
        const currentPhotoCount = galleryData.photosCount || 0;
        const availableSlots = Math.max(0, planLimits.photos - currentPhotoCount);
        
        if (validFiles.length >= availableSlots) {
          isValid = false;
          errorType = 'plan_limit_exceeded';
          rejectionReason = `Exceeds gallery limit of ${planLimits.photos} photos`;
          errorDetails.planLimit = planLimits.photos;
          errorDetails.currentCount = currentPhotoCount;
        }
      }
      ...
      if (isValid) {
        ...
        const errorInfo = getDetailedValidationError(file, errorType, errorDetails);
        rejectedFiles.push({
          file: file,
          reason: rejectionReason,
          errorInfo: errorInfo
        });
        console.log(`❌ File rejected: ${file.name} - ${rejectionReason}`);
      }
      
    } catch (error) {
      console.error(`Error validating file ${file.name}:`, error);
      const errorInfo = getDetailedValidationError(file, 'server_validation_failed', { serverError: error.message });
      rejectedFiles.push({
        file: file,
  ...
  // 6. Optional: Server-side validation for valid files
  if (validFiles.length > 0) {
    try {
      const serverValidation = await validateWithServer(validFiles, galleryId);
      if (serverValidation.warnings.length > 0) {
        warnings.push(...serverValidation.warnings);
      }
    } catch (error) {
      console.warn('Server validation failed, continuing with client validation:', error);
      warnings.push('Server validation unavailable, using client-side validation only');
    }
  }
  
  console.log(`📊 Validation complete: ${validFiles.length} valid, ${rejectedFiles.length} rejected`);
...
/**
 * REPLACE your existing handleFiles() function with this enhanced version
 */
async function handleFilesEnhanced(files) {
  ...
  try {
    ...
    showLoadingOverlay('Validating files...');
    
    // Run comprehensive validation
    const validationResult = await validateFilesComprehensively(
      imageFiles, 
      galleryId, 
      planLimits
    );
    ...
    window.filesToUpload = validationResult.validFiles;
    window.rejectedFiles = validationResult.rejectedFiles;
    
    hideLoadingOverlay();
    
    // Show warnings if any
    if (validationResult.warnings.length > 0) {
      validationResult.warnings.forEach(warning => showWarningMessage(warning));
    }
    ...
    if (validationResult.rejectedFiles.length > 0) {
      if (validationResult.validFiles.length === 0) {
        // All files rejected
        showErrorMessage(`All ${validationResult.rejectedFiles.length} files were rejected due to validation errors.`);
      } else {
        // Some files rejected
        showWarningMessage(`${validationResult.rejectedFiles.length} of ${imageFiles.length} files cannot be uploaded due to validation errors.`);
      }
      
      // Show detailed errors for first few rejected files
      validationResult.rejectedFiles.slice(0, 3).forEach(rejected => {
        if (rejected.errorInfo) {
          showEnhancedErrorMessage(rejected.errorInfo);
        }
      });
    }
    ...
    // Proceed to upload step if we have valid files
    ...
    showErrorMessage(`File validation failed: ${error.message}`);
  }
}
...
function handleFileSelect(event) {
  if (event.target && event.target.files) {
    handleFilesEnhanced(event.target.files);
  }
}
